    ...
    if (leafCluster) {
      centripetalOptions = {
        ...
        leaf: function leaf(node, nodes, edges) {
          var relativeNodesType = Utils.getRelativeNodesType(nodes, nodeClusterBy); // 找出与它关联的边的起点或终点出发的所有一度节点中同类型的叶子节点

          var _Utils$getCoreNodeAnd = Utils.getCoreNodeAndRelativeLeafNodes('leaf', node, edges, nodeClusterBy),
        ...
        center: function center(node, nodes, edges) {
          ...
          if (degree === 1) {
            ...
            var _Utils$getCoreNodeAnd2 = Utils.getCoreNodeAndRelativeLeafNodes('leaf', node, edges, nodeClusterBy),
